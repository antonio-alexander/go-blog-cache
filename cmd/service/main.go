...
func Main(pwd string, args []string, envs map[string]string, osSignal chan os.Signal) error {
	...
	logger.Info(correlationId, "service: go-blog-big-data v%s (%s) built from: %s\n",
	...
	defer func() {
		if err := sql.Close(correlationId); err != nil {
			logger.Error(correlationId, "error while closing sql: %s", err)
		}
	}()
	...
	defer func() {
		if err := logic.Close(correlationId); err != nil {
			logger.Error(correlationId, "error while closing logic: %s", err)
		}
	}()
