...
func scenarioRetry(ctx context.Context, envs map[string]string, logger utilities.Logger,
	timers utilities.Timers, clients ...client.Client) error {
	...
	scenarioSleep := 2 * time.Second
	if s := envs["SCENARIO_SLEEP_DURATION"]; s != "" {
		i, _ := strconv.Atoi(s)
		scenarioSleep = time.Duration(i) * time.Second
	}
	if len(clients) < minClients {
		return errors.New("not enough clients provided")
	}
	...
	for i := range clients {
		...
		go func(ctx context.Context, clientNumber int, client client.Client) {
			...
			sleepFx := func(ctx context.Context) error {
				if err := client.Sleep(ctx, scenarioSleep); err != nil {
					return err
				}
				return nil
			}
...
func Main(args []string, envs map[string]string, osSignal chan (os.Signal)) error {
	...
	case "retry":
		logger.Info(ctx, "executing %s scenario", scenario)
		if err := scenarioRetry(ctx, envs, logger, timers, clients...); err != nil {
			logger.Error(ctx, "error while executing %s scenario: %s", scenario, err)
		}
	}
	cancel()
