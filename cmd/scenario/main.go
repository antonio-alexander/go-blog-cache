			...
			for {
				...
				case <-tRead.C:
					if err := readEmployeeFx(ctx); err != nil {
						...
						logger.Info(ctx, "successfully read employee (%d)", empNo)
					}
				}
			}
		}(ctx, i, clients[i])
...
func scenarioCacheNotFound(ctx context.Context, envs map[string]string, logger utilities.Logger,
	timers utilities.Timers, clients ...client.Client) error {
	...
	if err := clients[0].EmployeeDelete(ctx, empNo); err != nil {
		return err
	}
	...
	for i := range clients {
		...
		go func(ctx context.Context, clientNumber int, client client.Client) {
			...
			correlationId := fmt.Sprintf("scenario_cache_not_found_%d", clientNumber)
			ctx = internal.CtxWithCorrelationId(ctx, correlationId)
			readEmployeeFx := func(ctx context.Context) error {
				if _, err := client.EmployeeRead(ctx, empNo); err != nil {
					return err
			...
			for {
				...
				case <-stop:
					return
				case <-tRead.C:
					timerIndex := timers.Start(correlationId)
					if err := readEmployeeFx(ctx); err != nil {
						switch {
						default:
							logger.Error(ctx, "error while reading employee: %s", err)
					...
					_ = timers.Stop(correlationId, timerIndex)
				}
			}
		}(ctx, i, clients[i])
	}
	...
	//print benchmarks
	totals := timers.ReadAll().Totals
	averages := timers.ReadAll().Averages
	for key := range averages {
		total, average := totals[key], averages[key]
		logger.Info(ctx, "%s total: %v, average: %v", key,
			time.Duration(total)*time.Nanosecond,
			time.Duration(average)*time.Nanosecond)
	}

	return nil
}

func scenarioSleepRetry(ctx context.Context, envs map[string]string, logger utilities.Logger,
	timers utilities.Timers, clients ...client.Client) error {
	...
	scenarioSleepDuration := 10 * time.Second
	if s := envs["SCENARIO_SLEEP_DURATION"]; s != "" {
		i, _ := strconv.Atoi(s)
		scenarioSleepDuration = time.Duration(i) * time.Second
	}
	scenarioSleepId := internal.GenerateId()
	if s := envs["SCENARIO_SLEEP_ID"]; s != "" {
		scenarioSleepId = s
	}
	if len(clients) < minClients {
		return errors.New("not enough clients provided")
	...
	for i := range clients {
		...
		go func(ctx context.Context, clientNumber int, client client.Client) {
			...
			correlationId := fmt.Sprintf("scenario_sleep_%d", clientNumber)
			ctx = internal.CtxWithCorrelationId(ctx, correlationId)
			...
			for {
				...
				case <-stop:
					return
				case <-tRead.C:
					timerId := timers.Start(correlationId)
					if err := sleepFx(ctx); err != nil {
						logger.Error(ctx, "error while sleeping: %s", err)
					}
					timers.Stop(correlationId, timerId)
				}
			}
		}(ctx, i, clients[i])
	}
...
func Main(args []string, envs map[string]string, osSignal chan (os.Signal)) error {
	...
	case "cache_not_found":
		logger.Info(ctx, "executing %s scenario", scenario)
		if err := scenarioCacheNotFound(ctx, envs, logger, timers, clients...); err != nil {
			logger.Error(ctx, "error while executing %s scenario: %s", scenario, err)
		}
	case "sleep_retry":
		logger.Info(ctx, "executing %s scenario", scenario)
		if err := scenarioSleepRetry(ctx, envs, logger, timers, clients...); err != nil {
			logger.Error(ctx, "error while executing %s scenario: %s", scenario, err)
		}
	}
	cancel()
