...
<!-- TODO: add something regarding goals or topics we talk about -->
...
<!-- REVIEW: I think this section makes sense, maybe we could create a TLDR? -->
...
<!-- REVIEW: I think you can clean this up -->

> natural keys for a given object _generally_ don't change over the lifetime of the object so when used as keys for caching and especially for caching search criteria can be cached __safely__. This is because natural keys describe the data in an intrinsic way and don't change.
...
<!-- REVIEW: originally, I didn't have this transition, so it may not make sense here

So, with all that theory out of the way, lets attempt to apply those theories to specific scenarios to give you an idea of how some of the questions asked above can have different answers depending on the scenario. -->
...
<!-- TODO:
- Talk about how to handle data for more than one tags
- Talk about how you almost always have to aggregate data from multiple tags
-->
...
How can load balancers and multiple instances of a service (or cache) affect the design of a cache?
...
> I think there's a lot of value in localized logging, knowing what operation is occurring in the background such as when an object has been invalidated (and why) or knowing if a request has been served from cache or from the source of truth. In addition being able to turn this logging on when needed versus always been on (or being able to view it retroactively) are also very valuable
