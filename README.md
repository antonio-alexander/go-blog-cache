...
![Cache Logic](./docs/images/cache_logic.png)
...
<!-- REVIEW: I feel like i've said this somewhere else that wasn't specific to an implementation -->

> No, although its safe to assume that there may be an implicit read-back when you mutate an object (so there's no benefit from the cache), there's no guarantee that it'll be read again and that there would be benefit to storing that version of the object in the cache. Its always best to populate the cache with things that are actively being read
